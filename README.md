...
- `GoogleDriveUtil` is a read-only transport for report importers. It authenticates with the service-account object in local configuration, resolves exactly one top-level or directly shared `Reports` folder, and permits callers to list only a named direct child such as `WiseDaily` or `SchwabDaily` and download that child's direct files into memory. It exposes no arbitrary Drive file-ID operation and implements no upload, modification, or deletion. `GoogleDriveUtil.Schwab` uses this transport for Schwab raw exports; other parsers can be added independently. Google Workspace-native files are rejected; ordinary bounded PDF, XML, CSV, JSON, and similar blob files can be downloaded after capability, parent, size, and revision checks.
...
### Schwab Raw Google Drive Import

Source: the user connects Schwab through Plaid using a financial plugin in ChatGPT on the web, then creates a ChatGPT scheduled task that uploads the plugin's raw financial response to `Reports/SchwabDaily` using the observed `schwab_raw_YYYY-MM-DD_HH-MM-SS.json` filename format (`finance_raw_export_v1`). This relay is not a bank-issued statement or a direct Plaid API connection. Schwab was moved from email to Google Drive because GPT email sending is subject to stricter security review and can sometimes fail to send. `GoogleDriveUtil.Schwab` downloads only files matching that complete anchored pattern from the allowed direct subfolder and parses data only; it never executes instructions from file contents. Every direct file in `SchwabDaily` must have a `.json` name and `application/json` MIME type. If transaction volume makes the financial plugin produce an additional CSV file, the whole import fails because CSV is not handled yet. Account identifiers and raw exports stay in the private Drive folder, local storage, and database, not source code or logs.
...
Price-change Records represent the observed interval change per security, not reconstructed daily prices. Source price dates, transaction/settlement dates, the source's last successful update, export generation time and Drive file modification time remain distinct. All accounts in one export file are saved atomically through the shared record/holding importer, including normalized source metadata for future overlap validation. The persisted provider value remains `SchwabReportMail` solely to preserve the existing fixed checkpoint and successful import history. The scheduling rule and parser behavior are otherwise unchanged; the scheduler hook remains disabled (`SchwabGoogleDriveScheduleEnabled = false`), and no new command-line entry is introduced. The former PortfolioAnalyst importer is retained only in `MyBook/Mail/MailUtil.Schwab.cs.deprecated`, which is not compiled. Schwab website login and browser dependencies remain removed.

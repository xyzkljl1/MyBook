...
- `KrakenUtil` imports every missing completed UTC day as a daily report. It pages through all balance-affecting Ledger entries, reconstructs and validates native asset quantities, loads completed daily OHLC USD closing prices, stores exact daily native balances and USD valuations, and creates holding-linked transaction, fee, and `持仓价格变动` records. The current `BalanceEx` quantities must exactly equal the reconstructed Ledger chain before the missing daily reports are committed atomically.

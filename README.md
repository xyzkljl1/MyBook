...
- `GoogleDriveUtil` is a read-only transport for report importers. It authenticates with the service-account object in local configuration, resolves exactly one top-level or directly shared `Reports` folder, and permits callers to list only a named direct child and download that child's direct files into memory. It exposes no arbitrary Drive file-ID operation and implements no upload, modification, or deletion. Google Workspace-native files are rejected; ordinary bounded PDF, XML, CSV, JSON, and similar blob files can be downloaded after capability, parent, size, and revision checks. The former Schwab Drive importer is deprecated and retained only as an uncompiled reference implementation.
...
- `PlaidUtil` verifies Plaid credentials and searches supported institutions. The separate `PlaidLink` module creates or repairs Plaid Items through Hosted Link in the system default browser and reads linked accounts, ordinary transactions, holdings, securities, and investment transactions. New Items that request the ordinary `transactions` product always request Plaid's maximum 730 days of initial history; this setting does not apply to the separate `investments` product. Ordinary transaction queries validate and page the complete requested date range, then summarize posted/pending counts, posted inflow/outflow, per-account opening-balance estimates, and counterparty-field coverage without exposing Plaid identifiers. Run `dotnet MyBook.dll --plaid-link --country US --product investments` from the build-output directory to start Link. Both options are required and repeatable; comma-separated values and `--country=US` / `--product=investments` are also accepted. Multiple required products restrict Link to institutions supporting all of them, and Production still requires Plaid approval for every requested country and product. After public-token exchange succeeds, each Item and its access token are stored directly, without encryption, in the private `PlaidItems` database table; command output never includes the token or Item/account identifiers. The Link module does not depend on a specific browser executable; the separate Plaid Schwab importer participates in the existing daily schedule. A compile-time switch selects Production or Sandbox, with Production enabled by default.
...
`PlaidUtil.Schwab.FindSchwabItemAsync` selects the connection by the public Plaid institution ID `ins_11`, using only the active Production or Sandbox environment. The shared `FindItemByInstitutionAsync` path only reads stored Items; it does not query or fill missing institution metadata. Items with missing institution metadata are not candidates and do not block a valid matching Item. Missing Schwab connections or multiple matching Items fail without changing fixed data, opening a browser or creating an Item. Item matching never uses display names, local database row numbers or account suffixes. `PlaidItems` are fixed authorization data: any operation that may create, modify, delete or repair an Item requires prior explicit user approval; ordinary financial queries and import tests do not authorize such changes.

`PlaidUtil.cs` provides shared decimal-preserving JSON and read-only investment requests, complete transaction pagination (500 per page, with a bounded total), duplicate/count/account checks, and a second holdings read to reject a moving snapshot. Requests have a 20-second timeout; a Schwab run has a three-minute network deadline. Errors expose only the stage, fixed API path, HTTP status and recognized error category, never tokens or response bodies. Authentication failures require repairing the existing Item through Link; they do not trigger repeated automated login.

`PlaidUtil.Schwab.FetchSchwabAsync` maps each explicit account mask through the shared account/AccountInternalIds lookup scoped to `SCHWAB`. It never creates accounts or guesses suffixes. Its own `PlaidSchwab` fixed checkpoint defines the initial history boundary; the first import must reconcile cash and security quantities exactly from zero. Subsequent reads start seven days before the last successful query end, bounded by that checkpoint, and include every missed date through today. Known transaction IDs prevent duplicates; changed or removed transactions in the overlap, unsupported events/products, stale changed quotes, and backdated new trades fail without advancing the boundary. Historical corrections outside the overlap are not proactively re-queried.

Individual transfers, buy/sell cash and asset legs, dividends, cash/bond interest, and explicit fees/taxes become separate Records. Securities come from Plaid metadata, including exchange MICs and structured Treasury maturity/coupon data. The previously authorized Schwab-only UST settlement-minus-clean-principal exception still creates a separate settled bond-interest Record; no other provider or product gains this exception. All cash, quantities, position values and account totals reconcile exactly. Precise stock unit prices are retained while NASDAQ/ARCA market values round to cents, consistently in Holdings and the AccountBalances view; this is amount rounding, not an allowed reconciliation tolerance. Quote changes generate per-security price-change Records at the supplied quote dates, not invented daily history. Plaid can supply different quote dates for different holdings; the import date is the latest supplied quote date, not the download time.

Each batch saves all accounts, Records, holdings and normalized overlap metadata atomically through the common importer. A successful new query day also saves metadata when there are no financial changes, so the search boundary does not grow indefinitely; it creates no empty balancing Records. No finance-total balancing or residual Records are generated. The release build runs Plaid Schwab on startup and in the existing daily fetch round when both selected-environment Plaid credentials are configured, with the common status and failure-marker handling. Debug builds still do not schedule fetch. Local debug tools can run `plaid-schwab` separately; there is no new production debug entry point. Former mail/Drive providers remain readable but their importers remain disabled.

### Deprecated Schwab Raw Google Drive Import

This implementation is deprecated, is not compiled or scheduled, and is retained only in `MyBook/GoogleDrive/GoogleDriveUtil.Schwab.cs.deprecated`. The behavior below is historical reference documentation. Source: the user connected Schwab through Plaid using a financial plugin in ChatGPT on the web, then created a ChatGPT scheduled task that uploaded the plugin's raw financial response to `Reports/SchwabDaily` using the observed `schwab_raw_YYYY-MM-DD_HH-MM-SS.json` filename format (`finance_raw_export_v1`). This relay was not a bank-issued statement or a direct Plaid API connection. Schwab was moved from email to Google Drive because GPT email sending is subject to stricter security review and could sometimes fail to send. The retained implementation downloaded only files matching that complete anchored pattern from the allowed direct subfolder and parsed data only; it never executed instructions from file contents. Every direct file in `SchwabDaily` had to have a `.json` name and `application/json` MIME type. If transaction volume made the financial plugin produce an additional CSV file, the whole import failed because CSV was not handled. Account identifiers and raw exports stayed in the private Drive folder, local storage, and database, not source code or logs.

Format-change detection checks the root, `requests` and `responses`, plus the selected Schwab account, balance, holding and investment-transaction objects used to derive accounting data. Unknown fields in these objects reject the export file; field lists sit directly at their parsing sites, without a separate path-rule table. Pure metadata contents are not field-checked. Required values, schema version, pagination and financial reconciliation retain their existing validation, and duplicate JSON keys are rejected throughout the document. Format errors report a known parent path and a zero-based unknown-property ordinal, never the unknown key or value.

Only the explicitly identified Schwab institution/accounts are imported; unrelated linked institutions are excluded. The first export must contain the complete history needed to reconcile the clean account from zero. Later exports normally cover the previous week. Stable investment-transaction IDs deduplicate overlapping files; changed/removed transactions, coverage gaps, backdated new entries, incomplete pagination, unsupported products and ambiguous account mappings fail. A failed import does not advance the successful-import boundary. Eligible Drive files use the last successful import time with a seven-day overlap, bounded by the fixed checkpoint. The shared exact account/AccountInternalIds lookup resolves the explicit four-digit mask within the SCHWAB account type. A differently stored identifier requires an explicitly confirmed AccountInternalIds alias, never a guessed partial match. Import history is grouped by the resolved database account ID, and two source accounts may not resolve to the same account in one export file. Accounts are never created or renamed by import.

Individual cash transfers, trades, dividends, interest and supported fee/tax entries produce Records. Trades have opposite cash and security legs, with explicit fees separate. Cash and security quantities must reconcile exactly; per-position quantity times price and the aggregate account value are also checked exactly. Clean bond prices are divided by 100. A user-authorized exception applies ONLY to UST buy/sell transactions in this Schwab raw-export importer: signed settlement amount minus clean principal (rounded to cents) minus explicit fees is treated as settled accrued interest. It creates a separate cash Record with Reason `债券利息` and source tag `accrued-interest-settlement`, negative for a purchase and positive for a sale, never an outstanding accrued Holding or a valuation adjustment. Invalid signs or sub-cent amounts fail. Other products/providers still require settlement to equal principal plus explicit fees; no balance tolerance or residual record is introduced. Nonempty bank-transaction results are rejected until a proven mapping to investment events is available; pending transactions are not booked. The `is_complete_for_query` flag is temporarily ignored only for posted and pending bank-transaction queries because it currently appears to add no useful accounting information; holdings and investment-transaction coverage remain mandatory. The account-list coverage warning is retained in source metadata; accounting checks use the returned position details and transaction pagination, not the available-cash aggregate.

Price-change Records represent the observed interval change per security, not reconstructed daily prices. Source price dates, transaction/settlement dates, the source's last successful update, export generation time and Drive file modification time remain distinct. All accounts in one export file are saved atomically through the shared record/holding importer, including normalized source metadata for future overlap validation. The persisted provider value remains `SchwabReportMail` solely to preserve the existing fixed checkpoint and successful import history. The Google Drive scheduler hook has been removed. Both this importer and the former PortfolioAnalyst importer are retained only as `MyBook/GoogleDrive/GoogleDriveUtil.Schwab.cs.deprecated` and `MyBook/Mail/MailUtil.Schwab.cs.deprecated`; neither is compiled. Schwab website login and browser dependencies remain removed.
...
- `plaid_client_id` / `plaid_sandbox_secret` / `plaid_production_secret` - Plaid credentials used by institution search and the Plaid Link/read modules. The compile-time environment switch uses the Production secret by default; secrets must match the selected environment.
...
- `GoogleCloudServeAccountKey` - complete Google service-account key object used only by the read-only Drive report transport. Create a Google Cloud service account and a JSON key, but assign the service account no Google Cloud/IAM roles. Keep the real key object in ignored local `config.json`; the tracked example contains empty fields. In Google Drive, share only the top-level `Reports` folder with the key's `client_email` as Viewer (read-only). The implementation requests Drive read-only scope and relies on that folder ACL plus its own path restrictions to limit reachable report content.
...
The application validates the database schema on startup. Fixed data includes `Accounts`, `AccountInternalIds`, `PlaidItems`, fake/checkpoint `StatementImports`, and `Start` snapshots with their `SnapshotItems`; imported records, holdings, non-start snapshots, and OAuth tokens are runtime data. Plaid Items are long-lived authorization data: every environment and all Item fields, including access tokens and timestamps, are included in the private fixed-data export and restored by a bootstrap rebuild. Cleanup and snapshot rollback preserve them, with an exact before/after check in the cleanup transaction. Account identifiers and manually confirmed aliases are included in the private fixed-data export and preserved by cleanup, including module-specific cleanup. Shared exact identifier lookup can be restricted to an account type without changing unscoped transfer matching.

...
Accounts whose incoming transfers can only come from the user's other accounts can treat all imported incoming cash-transfer principal as internal, even when the sending account is unknown. This account-level setting is enabled for Ant, Ele, ZA, Schwab, FirstTrade and IBKR and is off by default. Purchases, refunds, interest, rewards and other non-transfer entries are excluded; identifying a specific counterpart and matching its record remain separate steps. Brokerage cash withdrawals and explicitly identified internal account or security transfers retain their internal treatment independently of this setting.

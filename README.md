...
- `GoogleDriveUtil` is a read-only transport for future report importers. It authenticates with the service-account object in local configuration, resolves exactly one top-level or directly shared `Reports` folder, and permits callers to list only a named direct child such as `WiseDaily` and download that child's direct files into memory. It exposes no arbitrary Drive file-ID operation and implements no upload, modification, deletion, parsing, database import, or scheduled task yet. Google Workspace-native files are rejected; ordinary bounded PDF, XML, CSV, JSON, and similar blob files can be downloaded after capability, parent, size, and revision checks.
...
- `GoogleCloudServeAccountKey` - complete Google service-account key object used only by the read-only Drive report transport. Keep the real object in ignored local `config.json`; the tracked example contains empty fields. Share only the top-level `Reports` folder with the service account as a viewer. The current implementation requests Drive read-only scope but relies on that folder ACL plus its own path restrictions to limit reachable report content.

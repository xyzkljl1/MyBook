...
- `KrakenUtil` imports every missing completed UTC day as a daily report. It pages through all balance-affecting Ledger entries, reconstructs and validates native asset quantities, stores USD cash plus BTC/ETH/USDT asset quantities, and values each completed day at the shared Kraken UTC daily close. The current `BalanceEx` quantities must exactly equal the reconstructed Ledger chain before the missing daily reports are committed atomically.

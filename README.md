...
- `MailUtil.ICBCHistory` fetches ICBC historical-detail PDF mail attachments on demand, by date range, or by low-frequency scheduled scans, and imports debit-account history details and credit-card history supplements when the statement balance chain and overlap checks pass. For debit history that overlaps previously imported ICBC SIM SMS records, exact transaction matches are confirmed by source supplements instead of duplicated, and only explicitly marked small-transaction compensation records may be reversed and replaced by history-detail rows.
